-- Optimized Deep Work Tracker Schema
-- Only includes tables that are actually used in the current implementation

-- 1. Session Types Table (ESSENTIAL)
...
-- 2. Deep Work Sessions Table (ESSENTIAL)
CREATE TABLE IF NOT EXISTS deepwork_sessions (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  user_id UUID REFERENCES auth.users(id) ON DELETE CASCADE,
  session_type_id UUID REFERENCES deepwork_session_types(id),
  title VARCHAR(255) NOT NULL,
  description TEXT, -- Used for session reflection
  goal TEXT,
  tags TEXT[], -- array of tags
  planned_duration INTEGER NOT NULL, -- in minutes
  actual_duration INTEGER, -- in minutes (filled when completed)
  break_duration INTEGER DEFAULT 0, -- in minutes
  focus_score INTEGER, -- 0-100 (stored directly in sessions)
  distractions_count INTEGER DEFAULT 0, -- Added for tracking distractions
  tasks_completed INTEGER DEFAULT 0, -- Added for tracking completed tasks
  total_tasks INTEGER DEFAULT 0, -- Added for tracking total tasks
...
-- 3. Session Events Table (ESSENTIAL - for detailed tracking)
CREATE TABLE IF NOT EXISTS deepwork_session_events (
  ...
  data JSONB, -- additional event data (tasks_completed, total_tasks, reflection, etc.)
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
